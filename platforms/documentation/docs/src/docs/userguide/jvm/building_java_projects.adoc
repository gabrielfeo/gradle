 ...
 * the compilation classpath, including any required dependencies (via Gradle <<glossary.adoc#sub:terminology_configuration,configurations>>)
...
Either way, the Java Library Plugin adds a specific link:{groovyDslPath}/org.gradle.language.jvm.tasks.ProcessResources.html[ProcessResources] task for each source set that handles the processing of its associated resources.
...
Since `ProcessResources` is a subclass of the link:{groovyDslPath}/org.gradle.api.tasks.Copy.html[Copy] task, you can perform any of the processing described in the <<working_with_files.adoc#sec:copying_files,Working With Files>> chapter.
